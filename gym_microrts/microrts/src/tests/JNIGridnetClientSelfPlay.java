...
public class JNIGridnetClientSelfPlay {
     ...
     * @throws Exception
     */
    public JNIGridnetClientSelfPlay(RewardFunctionInterface[] a_rfs, String a_micrortsPath, String a_mapPath, UnitTypeTable a_uttP0, UnitTypeTable a_uttP1, boolean partial_obs) throws Exception{
        micrortsPath = a_micrortsPath;
        mapPath = a_mapPath;
        rfs = a_rfs;
        ...
        for (int i = 0; i < numPlayers; i++) {
            UnitTypeTable playerUTT = (i == 0) ? uttP0 : uttP1;
            ais[i] = new JNIAI(100, 0, playerUTT);
            masks[i] = new int[pgs.getHeight()][pgs.getWidth()][1+6+4+4+4+4+playerUTT.getUnitTypes().size()+maxAttackRadius*maxAttackRadius];
            rewards[i] = new double[rfs.length];
            dones[i] = new boolean[rfs.length];
            response[i] = new Response(null, null, null, null);
        }
