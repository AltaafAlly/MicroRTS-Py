...
public class JNIGridnetVecClient {
    ...
    /**
     * Constructor for asymmetric UTT support (selfplay + bot envs)
     * @param a_num_selfplayenvs
     * @param a_num_envs
     * @param a_max_steps
     * @param a_rfs
     * @param a_micrortsPath
     ...
     */
    public JNIGridnetVecClient(int a_num_selfplayenvs, int a_num_envs, int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths, 
    		AI[] a_ai2s, UnitTypeTable a_uttP0, UnitTypeTable a_uttP1, boolean partial_obs) throws Exception {
        ...
        for (int i = 0; i < a_num_envs; i++) {
            clients[i] = new JNIGridnetClient(a_rfs, a_micrortsPath, mapPaths[i], a_ai2s[i], a_uttP0, a_uttP1, partialObs);
        }
        selfPlayClients = new JNIGridnetClientSelfPlay[a_num_selfplayenvs];
        for (int i = 0; i < a_num_selfplayenvs; i++) {
            selfPlayClients[i] = new JNIGridnetClientSelfPlay(a_rfs, a_micrortsPath, mapPaths[i], a_uttP0, a_uttP1, partialObs);
        }
        responses = new Responses(null, null, null);
        rs = new Response[a_num_envs];
        reward = new double[a_num_envs][rfs.length];
        done = new boolean[a_num_envs][rfs.length];
        terminalReward1 = new double[rfs.length];
        terminalDone1 = new boolean[rfs.length];
    }
    
    /**
     * Constructor for asymmetric UTT support (bot vs bot)
     * @param a_max_steps
     * @param a_rfs
     * @param a_micrortsPath
     * @param a_mapPaths
     ...
     */
    public JNIGridnetVecClient(int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths,
        AI[] a_ai1s, AI[] a_ai2s, UnitTypeTable a_uttP0, UnitTypeTable a_uttP1, boolean partial_obs) throws Exception {
        maxSteps = a_max_steps;
        utt = a_uttP0; // For backward compatibility
        rfs = a_rfs;
        partialObs = partial_obs;
        ...
        botClients = new JNIBotClient[a_ai2s.length];
        for (int i = 0; i < botClients.length; i++) {
            botClients[i] = new JNIBotClient(a_rfs, a_micrortsPath, mapPaths[i], a_ai1s[i], a_ai2s[i], a_uttP0, a_uttP1, partialObs);
        }
        responses = new Responses(null, null, null);
        rs = new Response[a_ai2s.length];
        reward = new double[a_ai2s.length][rfs.length];
