...
public class GameState {
    ...
    // Placeholders for future asymmetric UTT support (default: both equal to 'utt')
    protected UnitTypeTable uttP0;
    protected UnitTypeTable uttP1;
    ...
    /**
     * Overloaded constructor for future asymmetric UTT support.
     * For now, also sets the default {@link #utt} to p0's table.
     */
    public GameState(PhysicalGameState a_pgs, UnitTypeTable p0, UnitTypeTable p1) {
        pgs = a_pgs;
        utt = p0;
        uttP0 = p0;
        uttP1 = p1 != null ? p1 : p0;
    ...
    /**
     * Returns the UTT for a given player (0 or 1). Currently returns the same
     * table for both players until asymmetric support is fully implemented.
     */
                ...
                for(UnitActionAssignment uaa:unitActions.values()) {
                    if (!uaa.action.resourceUsage(uaa.unit, pgs).consistentWith(ru, this)) {
                        ...
                        if (uaa.time==time) {
                            ...
                            switch(getUnitTypeTableForPlayer(p.m_a.getPlayer()).getMoveConflictResolutionStrategy()) {
                                default:
                                    System.err.println("Unknown move conflict resolution strategy in the UnitTypeTable!: " + getUnitTypeTableForPlayer(p.m_a.getPlayer()).getMoveConflictResolutionStrategy());
...
//            System.out.println("Executing action for " + u + " issued at time " + uaa.time + " with duration " + uaa.action.ETA(uaa.unit));
    ...
    /**
     * Clone this game state, replacing the active {@link UnitTypeTable}s for both players
     * @param new_uttP0 UTT for player 0
     * @param new_uttP1 UTT for player 1
     * @return the new GameState
     */
    public GameState cloneChangingUTTs(UnitTypeTable new_uttP0, UnitTypeTable new_uttP1)
    {
        GameState gs = clone();
        gs.utt = new_uttP0; // For backward compatibility, use P0's UTT as main
        gs.uttP0 = new_uttP0;
        gs.uttP1 = new_uttP1;
        for(Unit u:gs.getUnits()) {
            UnitTypeTable playerUTT = (u.getPlayer() == 0) ? new_uttP0 : new_uttP1;
            UnitType new_type = playerUTT.getUnitType(u.getType().name);
            if (new_type == null) return null;
            if (u.getHitPoints() == u.getType().hp) u.setHitPoints(new_type.hp);
            u.setType(new_type);
        }
