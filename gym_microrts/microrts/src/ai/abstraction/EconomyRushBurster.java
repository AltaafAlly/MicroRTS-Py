 ...
 * @author rubensolv
 */
public class EconomyRushBurster extends AbstractionLayerAI {
    ...
    public EconomyRushBurster(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public EconomyRushBurster(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
    ...
    @Override
    public AI clone() {
        return new EconomyRushBurster(utt, pf);
    }
        ...
        if (nbarracks != 0) {
            ...
            if (!otherResources.isEmpty()) {
                if (!freeWorkers.isEmpty()) {
                    ...
                    if (p.getResources() >= baseType.cost + resourcesUsed) {
                        Unit u = freeWorkers.remove(0);
                        if (buildIfNotAlreadyBuilding(u, baseType, otherResources.get(0).getX()-2, otherResources.get(0).getY()-2, reservedPositions, p, pgs)) {
                            resourcesUsed += baseType.cost;
                        }
                    }
                }
